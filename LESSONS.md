...
- 12 false "new product" pings from one flaky HTTP response. rarewaves' Klevu paging treated an empty page AFTER page 0 as the end of the catalogue, so a transient short page returned 100 of 112 products with errors == 0; main.py duly pruned the 12 missing entries, and the next run re-alerted every one as brand new. Only page 0's emptiness had been guarded. The fix is to compare what arrived against the total the API itself states — that covers every shape of short page without guessing which one went short — and ginza had the identical latent hole.
